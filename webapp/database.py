...
class MongoDBConnection:
    ...
    def connect(self) -> bool:
        ...
        try:
            logger.info(
                f"🔌 Connexion à MongoDB: {DatabaseConfig.MONGO_URI.split('@')[-1]}"
            )
...
class RedisConnection:
    ...
    def connect(self) -> bool:
        ...
        try:
            logger.info(
                f"🔌 Connexion à Redis: {DatabaseConfig.REDIS_HOST}:{DatabaseConfig.REDIS_PORT}"
            )
...
class DatabaseManager:
    ...
    def connect_all(self) -> Dict[str, bool]:
        ...
        results = {"mongodb": self.mongodb.connect(), "redis": self.redis.connect()}
...
if __name__ == "__main__":
    ...
    if success:
        ...
        print("\n" + "=" * 60)
        print(
            f"🏥 État global: {'✅ HEALTHY' if health['overall_healthy'] else '❌ UNHEALTHY'}"
        )
        print("=" * 60 + "\n")
